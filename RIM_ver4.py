...
# Custom Environment for Instance Memory Management with Memory Constraints
class InstanceMemoryEnv(gym.Env):
    ...
    def step(self, action):
            ...
            if action[i] == 0:  # Decrease views
                ...
                if self.current_memory_usage < self.total_memory_capacity:  # 총 메모리 용량을 초과하지 않을 때만 증가 가능
                    if self.current_views[i] < self.max_views:
                        self.current_views[i] += 1
                        reward += 1  # 올바른 할당에 대한 보상
                else:
                    reward -= 2  # 메모리 초과 상태에서 증가하려는 경우 페널티
        ...
        if self.current_memory_usage > self.total_memory_capacity:
            reward -= 5  # 메모리 초과 시 큰 페널티
        elif self.current_memory_usage > self.total_memory_capacity * 0.8:
            reward -= 1  # 메모리 사용량이 임계값에 근접한 경우 작은 페널티
...
model.save("instance_memory_model_with_scenario_and_memory_limit")

# Load the model (for future use)
model = PPO.load("instance_memory_model_with_scenario_and_memory_limit")
